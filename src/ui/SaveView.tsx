...
function describeSave(state: GameState): string {
  const people = `${state.roster.length} ${state.roster.length === 1 ? 'person' : 'people'}`
  const trips = `${state.nextExpeditionId - 1} expeditions`
  const saved = new Date(state.lastTick).toLocaleString()

  return `${people}, ${trips} — last played ${saved}`
}
...
export function SaveView({ state, onImport, onReset }: SaveViewProps) {
  ...
  // An import is held here until it is confirmed: it overwrites the town
  ...
  const confirmImport = () => {
    ...
    setStatus({
      tone: 'ok',
      message: 'Save loaded. The town is as you left it.',
    })
  }
  ...
  const reset = () => {
    ...
    setStatus({
      tone: 'ok',
      message: 'Wiped. The light is lit again, somewhere new.',
    })
  }
  ...
  return (
    <Stack gap="md">
      ...
      <Card withBorder padding="sm">
        ...
        <Text size="sm" c="dimmed" mt={4}>
          Replaces the town you are playing now. Time passed since the export is
          paid out on the way in, as if you had been away.
        </Text>
        <Group gap="xs" mt="sm">
          <FileButton
      ...
      <Card withBorder padding="sm">
        ...
        <Text size="sm" c="dimmed" mt={4}>
          Wipes this save and starts a new town. There is no undo — export first
