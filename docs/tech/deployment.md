...
`vite.config.ts` sets `base: '/idle/'` to match the GitHub Pages project subpath (`https://<user>.github.io/arpg-idle/`), and the PWA manifest's `start_url`/`scope` are pinned to `/arpg-idle/` for the same reason.
