...
GitHub Pages serves everything, `sw.js` included, with a fixed `Cache-Control: max-age=600` that cannot be overridden (see `docs/tech/deployment.md`). So an update check can see a copy of `sw.js` up to ~10 minutes stale. That's well inside the spec's 24-hour ceiling, and short relative to how often someone reopens an idle game, so it's accepted rather than worked around.
