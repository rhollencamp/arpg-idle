...
1. **Zone selection (simplest):** discovered locations are expedition
   ...
   village has more medicine, a quarry has more materials). _This is roughly
   what the MVP needs anyway, even as a list rather than a map._
...
5. **Landmarks & story:** ruins, a sunken cathedral, the _heart of the fog_.
...
- **Next version:** a real map built around **2 (outposts)** and
  **3 (siege camps)**:
  - _Siege camps_ connect expeditions to town defense: clearing a camp lowers
    siege pressure.
  - _Outposts_ are territory: lighting them extends the cleared area and adds
