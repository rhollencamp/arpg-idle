...
Two kinds of things come home:

1. **Supplies & materials** (common, the bulk of what's found): they feed the
   town economy.
2. **Gear** (occasional): what characters equip. **Truly useful gear is
   rare.**
...
A team can only bring home as much as it can carry (sum of Might plus bags).
A full pack is a valid reason to come home (a retreat rule). This keeps loot
from growing without limit and gives Might value outside combat.

## Salvage rules

Handling loot is a policy too:

- **MVP:** "Auto-salvage gear below [rarity]," applied when the team comes
  home. Salvage gives materials (and relic shards from Fine+).
- **Stash cap** (grows with a building). When full, new drops below the
  threshold get salvaged automatically, and anything else waits in an
  overflow slot for the player to decide.
- **Later:** rule lists like "keep if better than what anyone equipped
  has," "keep any Trinket with −oil use," "salvage duplicates."
